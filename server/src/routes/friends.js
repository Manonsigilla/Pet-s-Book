...
async function animalPreviews(ids) {
...
async function friendshipsOf(ids, status) {
...
router.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    const links = await friendshipsOf(mine, 'accepted');
    ...
    const previews = await animalPreviews([...new Set([...pairs.map((p) => p.otherId), ...mine])]);
    const seen = new Set();
    const friends = [];
...
router.post('/requests', requireAuth, async (req, res, next) => {
  try {
    ...
    const from = await db.prepare('SELECT id, owner_id FROM animals WHERE id = ?').get(fromAnimalId);
    const to = await db.prepare('SELECT id, owner_id, friend_policy FROM animals WHERE id = ?').get(toAnimalId);
...
router.get('/suggestions', requireAuth, async (req, res, next) => {
  try {
    ...
    const myFriends = new Set(
      (await friendshipsOf(mine, 'accepted')).map((f) => (mineSet.has(f.requesterId) ? f.addresseeId : f.requesterId)),
    );

    const myLocations = (await db.prepare(
    ...
    const allAccepted = await db.prepare(
